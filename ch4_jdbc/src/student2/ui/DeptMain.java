        ...
        while (run) {
            ...
            switch (menu) {
                ...
                case 6:
                    ...
                    System.out.println(result ? "교수정보 추가 성공" : "교수정보 추가 실패");
                    break;
                case 7:
                    // 수정할 고수정보 입력받기
                    professorDTO = info.updateProfessor(sc);
                    // 수정정보 서비스로 넘져준 후 결과 받기
                    result = professorService.updateProfessor(professorDTO);
                    System.out.println(result ? "교수정보 수정 성공" : "교수정보 수정 실패");
                    break;
                case 8:
                    // 삭제할 교수정보 입력받기
                    String profId = info.deleteProfessor(sc);
                    // 삭제정보 서비스로 넘겨준 후 결과받기
                    result = professorService.deleteProfessor(profId);
                    System.out.println(result ? "교수정보 삭제 성공" : "교수정보 삭제 실패");
                    break;
                case 9:
                    // 조회할 교수정보 입력받기
                    profId = info.selectProfessor(sc);
                    // 조회정보 서비스로 넘겨준 후 결과받기
